...
line_type find_line(FILE *inf, char str[], char **p)
{   int i;
...
int run_aes_avs_test(mode mm, type tt)
{   char  path[128], inbuf[1024], *p;
    ...
    for( i = 0 ; i < 3 ; ++i )
    {
        ...
        strcat(path, ".rsp");
        if(!(f = fopen(path, "r"))){
            printf("\nError. Can not open file: %s", path);
            return EXIT_FAILURE;
        }
...
{   int i, j;
    ...
    for( i = 0 ; i < 4 ; ++i )
        for( j = 0 ; j < 6 ; ++j){
            int ret = run_aes_avs_test((mode)i, (type)j);
            if(ret != EXIT_SUCCESS) status = EXIT_FAILURE;
        }
    printf("\n\n");
